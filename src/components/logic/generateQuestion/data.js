...
export const skill = (element) => {
  if (element.custom) {
    ...
    const firstDigit = element.firstDigit;
    const secondDigit = element.secondDigit;

    const customLevel = { add: { stage: {} }, subtract: { stage: {} }, multiply: { stage: {} }, divide: { stage: {} } };

    //reset everthing to 1d1d
    for (let index = 1; index <= 10; index++) {
      customLevel.add.stage[index] = [1, "add,", 1];
      customLevel.subtract.stage[index] = [1, "subtract,", 1];
      customLevel.multiply.stage[index] = [1, "multiply,", 1];
      customLevel.divide.stage[index] = [1, "divide,", 1];
    }

    //reset the operator to first,second digit for all stages
    for (let index = 1; index <= 10; index++) {
      customLevel[operator].stage[index] = [firstDigit, operator, secondDigit];
    }

    // console.log("From skill", customLevel);
...
export const level = (element) => {
  if (element.custom) {
    const operator = element.operator;
    const firstDigit = element.firstDigit;
    const secondDigit = element.secondDigit;

    //initalialise custom level to be a blank objext
    const customLevel = {
      1: {
        add: { min: 0, max: 0 },
    ...
    for (let index = 1; index <= 10; index++) {
      customLevel[index][operator].min = 1;
      customLevel[index][operator].max = index;
    }

    // console.log("From Level", customLevel);
    return { ...customLevel };
  } else
    return {
      1: {
