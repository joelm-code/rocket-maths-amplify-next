...
import { level as myLevel, skill as mySkill } from "./data";

export const generateQuestion = (quiz) => {
  const level = myLevel(quiz.levelCode);
  const skill = mySkill(quiz.levelCode);
  ...
  const difficultyLevel = (Math.random() * currentLevel * 2) / (mulitplier * currentLevel);
  ...
  console.log(`-- Generate Question --
    ...
    Question Code: ${newSkill}
    Current Level: ${currentLevel}
    Current Skill: ${quiz.skill[operator]}
    Generated Stage: ${generatedStage}
    Difficulty Level: ${difficultyLevel}    
    `);

  const { q1, q2, a } = buildQuestion(operator, generatedStage, quiz.levelCode);
  const symbol = {
    add: "+",
    subtract: "-",
    multiply: "x",
